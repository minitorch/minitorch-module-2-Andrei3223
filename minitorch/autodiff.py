...
def central_difference(f: Any, *vals: Any, arg: int = 0, epsilon: float = 1e-6) -> Any:
    ...
    for i, x in enumerate(vals):
        x_new.append(x if i != arg else x + epsilon)
    return (f(*x_new) - f(*vals)) / epsilon
...
def topological_sort(variable: Variable) -> Iterable[Variable]:
    ...
    def _DFS(variable: Variable) -> None:
        if variable.unique_id in used or variable.is_constant():
            return
        used.add(variable.unique_id)
        for parent in variable.parents:
            _DFS(parent)
        if not variable.is_constant():
            top_sort_list.append(variable)

    _DFS(variable)
    return top_sort_list[::-1]
