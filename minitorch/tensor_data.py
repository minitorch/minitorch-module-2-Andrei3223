...
def to_index(ordinal: int, shape: Shape, out_index: OutIndex) -> None:
    ...
    for i in range(len(shape)):
        out_index[len(shape) - i - 1] = (ordinal // alpha) % shape[len(shape) - i - 1]
        alpha *= shape[len(shape) - i - 1]
...
def shape_broadcast(shape1: UserShape, shape2: UserShape) -> UserShape:
    ...
    shape1 = np.concatenate((np.ones(out_size - size_1).astype(np.int32), shape1))
    shape2 = np.concatenate((np.ones(out_size - size_2).astype(np.int32), shape2))
